...
gulp.task('copy_sh', function () {
  gulp.src(_base_path + 'command.menu.sh')
    .pipe(gulp.dest(_base_dist_path))
    .pipe(notify({message: '===== command.menu.sh copy complete ====='}));
});
...
gulp.task('watch', () => {
  ...
  gulp.watch(_base_path + 'command.menu.sh', ['copy_sh']).on('change', function (event) {
    console.log('File ' + event.path + ' was ' + event.type + showinfo() + '')
  });
});
